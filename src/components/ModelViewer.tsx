...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  const content = useMemo<THREE.Object3D | null>(() => {
    ...
    if (ext === "obj") {
      console.warn("OBJ format support temporarily disabled");
      return null;
    }
  ...
  }, [content, initPitch, initYaw, pivot, autoFrame, camera, fadeIn, onLoaded]);
...
const ModelViewer: FC<ViewerProps> = ({
  ...
  return (
    <div
      ...
      className="relative rounded-2xl overflow-hidden border border-border bg-surface-dark"
    >
      {showScreenshotButton && (
        <button
          onClick={capture}
          className="absolute top-4 right-4 z-10 cursor-pointer px-4 py-2 border border-white/20 rounded-xl bg-surface/80 backdrop-blur-sm text-white hover:bg-accent-400 hover:border-accent-400 transition-all duration-300 text-sm font-display"
        >
          📸 Screenshot
        </button>
      )}
